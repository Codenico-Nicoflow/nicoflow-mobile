...
// Inline SVG (not emoji) so the rendered glyph matches web's lucide-react
// icon set exactly — same paths as calloutIconComponents.ts (Info,
// AlertTriangle, CheckCircle2, Lightbulb, Star, StickyNote, Flag,
// HelpCircle), hand-copied here since the WebView shell is a bundled string
// with no module resolution (see this file's header comment) and lucide's
// package ships as React components, not raw path strings.
...
function calloutIconSvg(icon: string): string {
...
<style>
  ...
  div[data-note-callout] > .callout-body p { margin: 0; }
...
const NoteCallout = Node.create({
  ...
  addNodeView() {
    return ({ node, HTMLAttributes, getPos, editor }) => {
      ...
      iconEl.innerHTML = calloutIconSvg(node.attrs.icon);
      ...
      return {
        ...
        update(updatedNode) {
          ...
          iconEl.innerHTML = calloutIconSvg(updatedNode.attrs.icon);
